...
class TreeView extends React.Component<TreeViewProps, TreeViewState> {
  ...
  drawTree(treeData: TreeData = this.state.treeData) {
    ...
    this.rootNode = treeMap(
      d3.hierarchy(root, node => node.children)
    ) as PointNode;
  ...
  update(source: PointNode) {
    const hasChildren = (children: PointNode[] | undefined) =>
    ...
    // Update the nodes...
    const node = this.g.selectAll('g.node').data<PointNode>(nodes, (d: any) => {
      return d.id || (d.id = ++i);
    });
    ...
    // Update the links...
    const link = this.g.selectAll('path.link').data(links, (d: any) => {
      return d.id;
    });
    ...
    linkUpdate
      ...
      .duration(this.duration)
      .attr('d', (d: PointNode) => {
        return d.parent ? diagonal(d, d.parent) : 0;
      });
