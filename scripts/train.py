...
def train_tree(name='tree', dataset='wine', max_depth=6, min_samples_leaf=2, **kwargs):
...
def train_rule(name='rule', dataset='breast_cancer', **kwargs):
    ...
    model_name = '-'.join([dataset, name])
    brl = RuleList(name=model_name, rule_maxlen=3, discretizer=discretizer, **kwargs)
...
def train_surrogate(model_file, is_global=True, sampling_rate=5, surrogate='rule'):
    ...
    if surrogate == 'rule':
        surrogate_model = RuleSurrogate(name=model_name, discretizer=data['discretizer'],
                                        rule_minlen=1, rule_maxlen=3, min_support=0.005,
                                        _lambda=100, nchain=30, eta=1, iters=30000)
    elif surrogate == 'tree':
...
if __name__ == '__main__':
    ...
    # train_nn(dataset='abalone', neurons=(30, 30, 30, 30, 30), tol=1e-6, alpha=0.00001, verbose=True)

    # train_surrogate('models/abalone-nn-30-30-30-30-30.mdl', surrogate='rule')
